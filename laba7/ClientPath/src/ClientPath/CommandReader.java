    ...
    public void executeCommand(String[] args) {
        switch (args[0]) {
            case "add":
                ...
                writer.writeCommandRequest(new Request(commandsList.get(0), laba, user));
                break;

            case "clear":
                writer.writeCommandRequest(new Request(commandsList.get(1), user));
                break;

            case "count_less_than_discipline":
                Discipline discipline = ((ComCountLessDiscipline) commandsList.get(2)).askArgs(pI);
                writer.writeCommandRequest(new Request(commandsList.get(2), discipline, user));
                break;

            case "print_field_descending_difficulty":
                writer.writeCommandRequest(new Request(commandsList.get(5), user));
                break;

            case "filter_by_difficulty":
                writer.writeCommandRequest(new Request(commandsList.get(3), args[1], user));
                break;

            case "help":
                writer.writeCommandRequest(new Request(commandsList.get(6), user));
                break;

            case "history":
                writer.writeCommandRequest(new Request(commandsList.get(7), user));
                break;

            case "info":
                writer.writeCommandRequest(new Request(commandsList.get(8), user));
                break;

            case "remove_by_id":
                writer.writeCommandRequest(new Request(commandsList.get(9), args[1], user));
                break;

            case "remove_head":
                writer.writeCommandRequest(new Request(commandsList.get(10), user));
                break;
            ...
            case "remove_lower":
                ...
                writer.writeCommandRequest(new Request(commandsList.get(11), laba1, user));
                break;

            case "execute_script":
                writer.writeCommandRequest(new Request(commandsList.get(12), user));
                scriptinput(args[1]);
                break;

            case "show":
                writer.writeCommandRequest(new Request(commandsList.get(13), user));
                break;

            case "update_by_id":
                if (writer.writeCommandRequest(new Request(commandsList.get(14), args[1], user)) == 1) {
                    LabWork laba2 = ((ComUpdateById) commandsList.get(14)).askArgs(args[1], pI, user);
                    laba2.setId(Long.parseLong(args[1]));
                    writer.writeCommandRequest(new Request(commandsList.get(14), laba2, user));
                }
                break;

            case "exit":
                writer.writeCommandRequest(new Request(commandsList.get(4), user));
                System.exit(0);
